...
export function ParameterSetDetailPage() {
  const { parameterSetId } = useParams<{ parameterSetId: string }>();
  
  // Find the parameter set by ID, or use the first one as default
  const initialParameterSet = parameterSets.find((ps) => ps.id === parameterSetId) || parameterSets[0];
  const [parameterSet, setParameterSet] = useState(initialParameterSet);
